...
#### Responsive design
The basic layout uses bootstrap grid (flexbox) in layout with different column sizes at different 
#### Light/dark theme 
...
...
```
The main `index.html` was modified to add a class, that will be used to switch modes
```
<body class="themeA">
```
...
* :hover uses simple transitions, but the selector used is `.buttonGlobal:hover:enabled` so that hover doesn't have any effect is the button is disabled. I might add a color transform for :disabled so that disabled buttons (on navigation menu for instance) are different.

#### Search button

This is a variant on the custom buttons. When disabled (set the `disabled` attribute) the button disappears and text telling the user to start typing in the form input boxes appears. This was done using a `::before` psuedo-property. It would have been simpler to render text/search/searching based on a state variable. 

#### Debug

Debug was generally difficult because each part of the design has it's own strange way of running and reporting errors so it took a long time to find the root cause
